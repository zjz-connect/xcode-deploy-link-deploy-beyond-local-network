...
func (s *Session) Screenshot(ctx context.Context) ([]byte, error) {
	...
	stop := context.AfterFunc(ctx, service.Close)
	defer func() { stop(); service.Close() }()
...
func (d *Daemon) screenshot(ctx context.Context) ([]byte, error) {
	if !d.operationMu.TryLock() {
		return nil, coded("session_busy", "another operation owns the device session", nil)
	}
	defer d.operationMu.Unlock()
	session := d.currentSession()
	if session == nil {
		return nil, coded("session_not_active", "no existing RemotePairing session is available", nil)
	}
	...
	return session.Screenshot(captureCtx)
}
